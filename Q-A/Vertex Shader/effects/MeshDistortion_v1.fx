...
float distance;
texture myTexture <string uiname="myTexture";>;
sampler mySamp = sampler_state    	   //sampler for doing the texture-lookup
{ 
    Texture   = (myTexture);          //apply a texture to the sampler
    ...
    float4 TexCd : TEXCOORD0)
{
	...
	//float y=distance;//*PosO.y;
	//float pos=float2(PosO.x, PosO.y);
	
	//float4 texColor = tex2Dlod(mySamp, TexCd);
	
	//Adjust Position
	PosO+=float4(NormO.xyz, 0.0)*distance;
